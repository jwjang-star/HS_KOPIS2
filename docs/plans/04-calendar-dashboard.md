...
**상태: 1차 구현 완료 + 로컬 브라우저 검증 완료 (2026-09-02). 프로덕션 미배포. 날짜 클릭 리스트 "공연/축제 2단 분리 + 가로 그리드" 개선은 다음 세션 예정.**

2026-08-07 설계 제안 → 2026-09-02 사용자 재검토 후 착수. 미정이던 2건 확정: ① 장르/상태 필터 = 그리드 뷰와 공유 ② 날짜 클릭 = 정보 열람만(체크박스·선택발송 없음).
...
1. **날짜 클릭 리스트 개선** — 현재는 공연+축제가 한 줄로 세로 스택(최대 ~90장). "공연 섹션 / 축제 섹션" 세로로 나누고, 각 섹션 안에서 카드를 가로 그리드(`repeat(auto-fill,minmax(240px,1fr))`, 넓으면 4열)로. `renderCalDayList()` 재작성(innerHTML 문자열 → DOM 빌드) + CSS `.cal-daylist-sec`/`.cal-daylist-grid` 추가 + 기존 `.cal-daylist .pitem` margin-bottom 제거. 좌우 2칼럼(공연|축제)은 건수 불균형(추석 24 = 공연1:축제25) 때문에 기각. 공연 섹션 NEW/기존 분리는 안 하는 쪽(캘린더 열람 전용).
2. **배포** — 위 개선까지 끝나면 `git push origin master:main` (백엔드 무변경, GitHub Pages만 반영)
3. `CAL_MAX_PAGES=3`(300건) 캡 실사용 적정성 실측 후 확정
