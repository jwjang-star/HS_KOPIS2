...
**상태: 1차 구현 + 날짜 리스트 2단화까지 완료 + 로컬 브라우저 검증 완료 (2026-09-03). 프로덕션 미배포 — 배포 승인 대기.**

2026-08-07 설계 제안 → 2026-09-02 사용자 재검토 후 착수 → 2026-09-03 날짜 클릭 리스트 개선. 미정이던 2건 확정: ① 장르/상태 필터 = 그리드 뷰와 공유 ② 날짜 클릭 = 정보 열람만(체크박스·선택발송 없음).
...
1. **배포** — `git push origin master:main` (백엔드 무변경, GitHub Pages만 반영). 승인 대기.
2. `CAL_MAX_PAGES=3`(300건) 캡 실사용 적정성 실측 후 확정
